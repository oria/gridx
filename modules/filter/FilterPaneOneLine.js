define([
	...
	"dijit/form/ComboBox"
], function(declare, lang, array, dom, css, string, query, registry, metrics, DistinctComboBoxMenu, Filter, template, i18n){
	...
	return declare([dijit.layout.ContentPane], {
		...
		_initFields: function(){
			this.sltColumn = registry.byNode(query('>div>.dijitSelect', this.domNode)[0]);
			this.sltCondition = registry.byNode(query('>div>.dijitSelect', this.domNode)[1]);
		...
		_initCloseButton: function(){
			...
			var closeButton = registry.byNode(query('.gridxFilterPaneRemoveBtn', this.domNode)[0]);
			var addButton = registry.byNode(query('.gridxFilterPaneAddBtn', this.domNode)[0]);
			this.connect(closeButton, 'onClick', 'close');
			this.connect(addButton, 'onClick', function(){
				this.grid.filterBar._filterDialog.addRule(this.domNode);
			});
		},
		...
		_updateTitle: function(){
			...
			if(value && (condition !== 'range' || (value.start && value.end))){
				...
				var ruleNumber = array.indexOf(this.domNode.parentNode.childNodes, this.domNode) + 1;
