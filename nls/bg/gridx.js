define({     
//Body
	loadingInfo: "Зареждане...",
	emptyInfo: "Няма елементи за показване",
	loadFailInfo: "Неуспешно зареждане на данни!",
	loadMore: "Зареждане на повече",
	loadMoreLoading: "Зареждане...",
	loadPrevious: "Зареди предишни",
	loadPreviousLoading: "Зареждане...",

//FilterBar
	"clearFilterDialogTitle": "Изчисти филтър",
	"filterDefDialogTitle": "Филтър",
	"defaultRuleTitle": "Правило",
	"ruleTitleTemplate": "Правило ${ruleNumber}",
	"noFilterApplied": "Не е приложен филтър.",
	"defineFilter": "Определи филтър",
	"conditionEqual": "равен",
	"conditionNotEqual": "не е равен",
	"conditionLess": "по-малък от",
	"conditionLessEqual": "по-малък от или равен",
	"conditionGreater": "по-голям от",
	"conditionGreaterEqual": "по-голям от или равен",
	"conditionContain": "съдържа",
	"conditionIs": "е",
	"conditionStartWith": "започва с",
	"conditionEndWith": "завършва с",
	"conditionNotContain": "не съдържа",
	"conditionIsNot": "не е",
	"conditionNotStartWith": "не започва с",
	"conditionNotEndWith": "не завършва с",
	"conditionBefore": "преди",
	"conditionAfter": "след",
	"conditionRange": "обхват",
	"conditionIsEmpty": "празен",
	"all": "всички",
	"any": "които и да било",
	"relationAll": "всички правила",
	"waiRelAll": "Комбинирай всички следни правила:",
	"relationAny": "всяко правило",
	"waiRelAny": "Комбинирай които и да било от следните правила:",
	"relationMsgFront": "Комбинирай",
	"relationMsgTail": "",
	"and": "и",
	"or": "или",
	"addRuleButton": "Добави правило",
	"waiAddRuleButton": "Добави ново правило",
	"removeRuleButton": "Премахни правило",
	"waiRemoveRuleButtonTemplate": "Премахни правило ${0}",
	"addRuleButton": "Добави правило за филтър",
	"cancelButton": "Отмени",
	"waiCancelButton": "Отмени този диалог",
	"clearButton": "Изчисти",
	"waiClearButton": "Изчисти филтъра",
	"filterButton": "Филтър",
	"waiFilterButton": "Подай филтър",
	"columnSelectLabel": "Колона:",
	"waiColumnSelectTemplate": "Колона за правило ${0}",
	"conditionSelectLabel": "Условие:",
	"waiConditionSelectTemplate": "Условие за правило ${0}",
	"valueBoxLabel": "Стойност:",
	"waiValueBoxTemplate": "Въведи стойност за филтриране на правило ${0}",
	"rangeTo": "до",
	"rangeTemplate": "от ${0} до ${1}",
	"statusTipHeaderColumn": "Колона",
	"statusTipHeaderCondition": "Правила",
	"statusTipTitle": "Лента с филтри",
	"statusTipMsg": "Щракнете върху лентата с филтри тук, за да филтрирате стойностите в ${0}.",
	"anycolumn": "която и да било колона",
	"statusTipTitleNoFilter": "Лента с филтри",
	"statusTipTitleHasFilter": "Филтър",
	"statusTipRelPre": "Комбинирай",
	"statusTipRelPost": "правила.",
	"statusTipHeaderAll": "Комбинирай всички правила.",
	"statusTipHeaderAny": "Комбинирай които и да било правила.",
	"defaultItemsName": "елементи",
	"filterBarMsgHasFilterTemplate": "Показани ${0} от ${1} ${2}.",
	"filterBarMsgNoFilterTemplate": "Не е приложен филтър",
	"filterBarDefButton": "Определи филтър",
	"waiFilterBarDefButton": "Филтрирай таблицата",
	"a11yFilterBarDefButton": "Филтър...",
	"filterBarClearButton": "Изчисти филтър",
	"waiFilterBarClearButton": "Изчисти филтъра",
	"closeFilterBarBtn": "Затвори лентата с филтри",
	"clearFilterMsg": "Това ще премахне филтъра и ще покаже всички налични записи.",
	"anyColumnOption": "Която и да било колона",
	"trueLabel": "Вярно",
	"falseLabel": "Невярно",
	"radioTrueLabel": "Стойността е вярна",
	"radioFalseLabel": "Стойността е невярна",
	"beginTimeRangeLabel": "Стойност за времеви обхват за начало",
	"endTimeRangeLabel": "Стойност за времеви обхват за край",
	"beginDateRangeLabel": "Стойност за обхват от дати за начало",
	"endDateRangeLabel": "Стойност за обхват от дати за край",
	"startsWithExpr": "${0}*",

//NestedSort
	singleSort: "Единично сортиране",
	nestedSort: "Вградено сортиране",
	ascending: "Щракнете, за да сортирате във възходящ ред",
	descending: "Щракнете, за да сортирате в низходящ ред",
	sortingState: "${0} - ${1}",
	unsorted: "Не сортирайте тази колона",
	waiSingleSortLabel: "${0} - е сортирана по ${1}. Изберете да сортирате по ${2}",
	waiNestedSortLabel:"${0} - е вложена по ${1}. Изберете да вложите по ${2}",

//PaginationBar
	pagerWai: 'Пейджър',

	pageIndex: '${0}',
	pageIndexTitle: 'Страница ${0}',

	firstPageTitle: 'Първа страница',
	prevPageTitle: 'Предишна страница',
	nextPageTitle: 'Следваща страница',
	lastPageTitle: 'Последна страница',

	pageSize: '${0}',
	pageSizeTitle: '${0} елементи на страница',
	pageSizeAll: 'Всички',
	pageSizeAllTitle: 'Всички елементи',

	description: '${0} - ${1} от ${2} елементи.',
	descriptionEmpty: 'Мрежата е празна.',

	summary: 'Общо: ${0}',
	summaryWithSelection: 'Общо: ${0} Избрани: ${1}',

	gotoBtnTitle: 'Отиди до конкретна страница',

	gotoDialogTitle: 'Отиди до страница',
	gotoDialogMainMsg: 'Укажи номер на страница:',
	gotoDialogPageCount: '(${0} страници)',
	gotoDialogOKBtn: 'Отиди',
	gotoDialogCancelBtn: 'Отмени',
	// for drop down pagination bar
	pageLabel: 'Страница',
	pageSizeLabel: 'Редове',

//QuickFilter
	filterLabel: 'Филтър',
	clearButtonTitle: 'Изчисти филтър',
	buildFilterMenuLabel: 'Изгради филтър&hellip;',
	apply: 'Приложи филтър',

//Sort
	helpMsg: '${0} - Щракнете за сортиране или контрол-щракнете за добавяне на сортиране',
	singleHelpMsg: '${0} - Щракнете за сортиране',
	priorityOrder: 'сортиране на свойство ${0}',

//SummaryBar
	summaryTotal: 'Общо: ${0}',
	summarySelected: 'Избрани: ${0}',
	summaryRange: 'Обхват: ${0}-${1}',	//need translation

//Other
	indirectSelectAll: "Натиснете SPACE, за да изберете всички.",	//need translation
	indirectDeselectAll: "Натиснете SPACE, за да премахнете избирането на всички.",	//need translation
	treeExpanded: "Контрол + клавиш с лява стралка за сгъване на този ред.",	//need translation
	treeCollapsed: "Контрол + клавиш с дясна стрелка за разгъване на този ред."	//need translation
});

